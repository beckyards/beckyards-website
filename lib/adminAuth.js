...
export async function getAdminUser() {
  ...
  try {
    ...
    if (result.error) {
      console.error('[getAdminUser] auth.getUser() returned an error:', JSON.stringify(result.error, Object.getOwnPropertyNames(result.error)));
    }
    user = result.data.user;
  } catch (err) {
    console.error('[getAdminUser] auth.getUser() threw:', JSON.stringify(err, Object.getOwnPropertyNames(err)), err?.stack);
    throw err;
  }
...
export async function requireAdminClient() {
  ...
  try {
    ...
    console.error('[requireAdminClient] createAdminClient() threw:', JSON.stringify(err, Object.getOwnPropertyNames(err)), err?.stack);
